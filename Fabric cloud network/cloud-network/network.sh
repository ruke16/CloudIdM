...
# and applications. The cloud network consists of two organizations with one
...
# Versions of fabric known not to work with the cloud network
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $NONWORKING_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      fatalln "Local Fabric binary version of $LOCAL_VERSION does not match the versions supported by the cloud network."
    fi

    infoln "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      fatalln "Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match the versions supported by the cloud network."
    fi
  done
...
# certificates, and MSP folders that are needed to create the cloud network in the
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Create CSP 1 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-CSP1.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Create CSP 2 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-CSP2.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/CSP1/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generate CCP files for CSP 1 and CSP 2"
...
# file for creating the network is "docker-compose-cloud-net.yaml" in the ``docker``
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/CSP1/msp organizations/fabric-ca/CSP1/tls-cert.pem organizations/fabric-ca/CSP1/ca-cert.pem organizations/fabric-ca/CSP1/IssuerPublicKey organizations/fabric-ca/CSP1/IssuerRevocationPublicKey organizations/fabric-ca/CSP1/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/CSP2/msp organizations/fabric-ca/CSP2/tls-cert.pem organizations/fabric-ca/CSP2/ca-cert.pem organizations/fabric-ca/CSP2/IssuerPublicKey organizations/fabric-ca/CSP2/IssuerRevocationPublicKey organizations/fabric-ca/CSP2/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/ordererOrg/msp organizations/fabric-ca/ordererOrg/tls-cert.pem organizations/fabric-ca/ordererOrg/ca-cert.pem organizations/fabric-ca/ordererOrg/IssuerPublicKey organizations/fabric-ca/ordererOrg/IssuerRevocationPublicKey organizations/fabric-ca/ordererOrg/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf addCSP3/fabric-ca/CSP3/msp addCSP3/fabric-ca/CSP3/tls-cert.pem addCSP3/fabric-ca/CSP3/ca-cert.pem addCSP3/fabric-ca/CSP3/IssuerPublicKey addCSP3/fabric-ca/CSP3/IssuerRevocationPublicKey addCSP3/fabric-ca/CSP3/fabric-ca-server.db'
...
COMPOSE_FILE_BASE=docker/docker-compose-cloud-net.yaml
# docker-compose.yaml file if you are using couchdb
COMPOSE_FILE_COUCH=docker/docker-compose-couch.yaml
# certificate authorities compose file
COMPOSE_FILE_CA=docker/docker-compose-ca.yaml
# use this as the docker compose couch file for org3
COMPOSE_FILE_COUCH_ORG3=addCSP3/docker/docker-compose-couch-CSP3.yaml
# use this as the default docker-compose yaml definition for org3
COMPOSE_FILE_ORG3=addCSP3/docker/docker-compose-CSP3.yaml
#
