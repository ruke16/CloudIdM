...
createAnchorPeerTx() {
	...
	for orgmsp in CSP1MSP CSP2MSP; do
...
createChannel() {
	...
	while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
		sleep $DELAY
		set -x
		peer channel create -o localhost:7050 -c $CHANNEL_NAME --ordererTLSHostnameOverride orderer.cloud.com -f ./channel-artifacts/${CHANNEL_NAME}.tx --outputBlock ./channel-artifacts/${CHANNEL_NAME}.block --tls --cafile $ORDERER_CA >&log.txt
		res=$?
		{ set +x; } 2>/dev/null
		let rc=$res
...
joinChannel() {
  ...
  setGlobals $ORG
	...
	cat log.txt
	verifyResult $res "After $MAX_RETRY attempts, peer0.CSP${ORG} has failed to join channel '$CHANNEL_NAME' "
}
...
infoln "Join CSP 1 peers to the channel..."
joinChannel 1
infoln "Join CSP 2 peers to the channel..."
joinChannel 2

## Set the anchor peers for each org in the channel
infoln "Updating anchor peers for CSP 1..."
updateAnchorPeers 1
infoln "Updating anchor peers for CSP 2..."
updateAnchorPeers 2
